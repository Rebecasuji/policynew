...
UPDATE policies 
SET content = 'Official shifts are: Shift 1: 09:30 AM – 06:30 PM, Shift 2: 10:00 AM – 07:00 PM, Shift 3: 11:00 AM – 08:00 PM. Each shift includes a one-hour break. Employees must report at designated shift start time with a 20-minute buffer period.'
WHERE title = 'Office Timings & Buffer Time';

UPDATE policies 
SET content = 'Every month, employees who arrive before their shift start time and leave after their shift end time every working day—without taking any leave (including sick leave and paid leave)—will receive a 5% bonus on their monthly salary. This applies only to employees with a salary not exceeding ₹25,000 per month.'
WHERE title = '100% Attendance Reward';

UPDATE policies 
SET content = 'Employees are allowed up to 3 late arrivals per month without salary deduction. From the 4th to 10th late arrival, deduction is based on hourly wage loss. Late arrivals exceeding 10 times result in half-day salary deduction retroactively from the first late arrival of the month.'
WHERE title = 'Late Arrival Policy & Salary Deductions';

UPDATE policies 
SET content = 'A half-day will be applied if an employee arrives after 4 hours from their shift start time. Salary will be calculated based on hours worked if less than 5 hours, which is the minimum for half-day salary.'
WHERE title = 'Half-Day Policy';

UPDATE policies 
SET content = 'Employees are not encouraged to work beyond shift end times unless required. Overtime is calculated only after 9 hours of work from punched-in time. Must be approved by reporting manager via email or Cliq.'
WHERE title = 'Overtime Policy';

UPDATE policies 
SET content = 'Employees must adhere to these policies to maintain discipline and professionalism. Any disputes regarding attendance, salary deductions, or overtime must be addressed with HR.'
